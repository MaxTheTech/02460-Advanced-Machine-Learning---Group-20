...
class VAE(nn.Module):
    ...
    def __init__(self, prior, decoder, encoder, num_decoders=1):
        ...
        if num_decoders == 1:
            ...
            self.decoder = nn.ModuleList([GaussianDecoder(new_decoder()) for _ in range(num_decoders)])
    def elbo(self, x):
        """
    ...
    def sample(self, n_samples=1):
        ...
        if self.num_decoders == 1:
            ...
            samples = []
            for decoder in self.decoder:
                samples.append(decoder(z).sample())
...
class GeodesicSolver(nn.Module):
    def __init__(self, model, c0, c1, T=1000):
        ...
        self.T = T
        self.model = model
        self.is_ensemble = model.num_decoders > 1
    ...
    def pullback_metric(self, model, z):
        ...
        G = J.mT @ J # [B, M, M]
        return G
    def  model_average_curve_energy(self, decoders, c, num_samples=20):
        # where fl and fk denotes decoder ensemble members drawn uniformly. This expectation can be approximated using Monte Carlo.
        #sample 2 decoders
        total_energy = 0
        for _ in range(num_samples):
            decoder_a, decoder_b = random.choices(decoders, k=2)        
            img_l = decoder_a.decoder_net(c[:-1]).reshape(c.shape[0] - 1, -1)
            img_k = decoder_b.decoder_net(c[1:]).reshape(c.shape[0] - 1, -1)
            
            total_energy += torch.sum((img_l - img_k) ** 2)
    ...
    def forward(self):
        ...
        if not self.is_ensemble:
            ...
            energy = self.model_average_curve_energy(self.model.decoder, c)
        return energy
    ...
    def geodesic_distance(self):
        ...
        if not self.is_ensemble:
            ...
            for i in range(len(c) - 1):
                expected_sq_dist = 0
                for l in range(self.model.num_decoders):
                    for k in range(self.model.num_decoders):
                        img_l = self.model.decoder[l].decoder_net(c[i:i+1]).reshape(1, -1)
                        img_k = self.model.decoder[k].decoder_net(c[i+1:i+2]).reshape(1, -1)
                        expected_sq_dist += torch.sum((img_l - img_k) ** 2)
                expected_sq_dist /= (self.model.num_decoders ** 2)
                total_distance += torch.sqrt(expected_sq_dist)
