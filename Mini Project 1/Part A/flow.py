...
class MaskedCouplingLayer(nn.Module):
    ...
    def forward(self, z):
        ...
        log_det_J = torch.sum((1 - self.mask) * self.scale_net(x_masked), dim=1)
        x = x_masked + (1 - self.mask) * (x * torch.exp(self.scale_net(x_masked)) + self.translation_net(x_masked))
        return x, log_det_J
    ...
    def inverse(self, x):
        ...
        z = z_masked + (1 - self.mask) * ((z - self.translation_net(z_masked)) * torch.exp(- self.scale_net(z_masked)))
        return z, log_det_J
